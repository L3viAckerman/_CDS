...
void DetectLane::update(const Mat &src)
{
    ...
    while (leftLane[i] == DetectLane::null && rightLane[i] == DetectLane::null)
        {
            i--;
            if (i < 0)
                return;
        }
        if (leftLane[i] != DetectLane::null && rightLane[i] != DetectLane::null)
        {
            CENTER = (leftLane[i] * 0.5 + rightLane[i] * 0.5);
        }
        else if (leftLane[i] != DetectLane::null)
        {
            CENTER = leftLane[i] + Point(laneWidth * 0.50, 0);
        }
        else
        {
            CENTER = rightLane[i] - Point(laneWidth * 0.50, 0);
        }
        circle(lane, CENTER, 1, (0, 255, 255), LINE_8, 0);
...
Mat DetectLane::preProcess(const Mat &src)
{
    ...
    for (int i = 0; i < imgThresholded.rows / 3; i++)
    {
...
vector<vector<Point>> DetectLane::centerRoadSide(const vector<Mat> &src, int dir)
{
    ...
    for (int i = 0; i < inputN; i++)
    {
        ...
        drawContours(src[i], cnts, -1, Scalar(0, 255, 255));
...
void DetectLane::detectLeftRight(const vector<vector<Point>> &points)
{
    ...
    bool detectSuceess = true;
    if (lane1.size() < 5 || lane2.size() < 5 || lane2[lane2.size() - 1].y < minStartPostion)
    {
        detectSuceess = false;
    }
    ...
    fitLine(subLane1, line1, CV_DIST_L2, 0, 0.01, 0.01);
    fitLine(subLane2, line2, 2, 0, 0.01, 0.01);
    float lane1X = (BIRDVIEW_HEIGHT - line1[3]) * line1[0] / line1[1] + line1[2];
    float lane2X = (BIRDVIEW_HEIGHT - line2[3]) * line2[0] / line2[1] + line2[2];
    if (abs(lane1X - lane2X) < minDistanceBetween2Lane)
        detectSuceess = false;
    if (detectSuceess)
    {
        if (lane1X < lane2X)
